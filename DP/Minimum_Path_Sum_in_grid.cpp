...
public:
    int minSumPathUtil(int i, int j, vector<vector<int>>& matrix,
        ...
        // Base cases
        if (i == 0 && j == 0)
            return matrix[0][0]; // If we are at the top-left corner, the
                                 // minimum path sum is the value at (0, 0)
        if (i < 0 || j < 0)
            return 1e9; // If we go out of bounds, return a large value to
    ...
    int minPathSum(vector<vector<int>>& grid) {
        ...
        int m = grid[0].size();
        vector<vector<int>> dp(
            n, vector<int>(m, -1)); // DP table to memoize results
        return minSumPathUtil(n - 1, m - 1, grid,
                              dp); // Start from the bottom-right corner
    }
};
